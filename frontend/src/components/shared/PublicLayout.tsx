      ...
      <nav className="bg-teal-700 text-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex gap-6 text-sm font-medium items-center">
          ...
          {isAuthenticated && isAdmin && (
            <Link to="/admin" className="hover:text-teal-200 transition-colors">Admin Portal</Link>
          )}
          {isAuthenticated && !isAdmin && (
            <Link to="/donor" className="hover:text-teal-200 transition-colors">Donor Portal</Link>
          )}
          {!isAuthenticated && (
